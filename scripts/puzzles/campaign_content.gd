...
static func item(id: String, kind: String, x: float, y: float = 0.08) -> Dictionary:
	return {"id":id, "kind":kind, "x":x, "y":y, "z":0.5}
...
static func rooms(id: String) -> Array:
	match id:
		"workshop": return [
			room("藏在木箱之后", "找回维修通道的电力", [control("hatch","latch",16,["fuse_socket"]),control("fuse_socket","socket",12,[],{"accept":"fuse"})], [item("fuse","fuse",8.2)], ["hatch"], ["木箱后有一束微弱的蓝光。","箱子既能推，也能向后拉；保险丝可以带走。","按住互动键拉开箱子，把保险丝装进蓝色插座，再打开维修闩。"], {"crate":7.0,"crate_target":3.0}),
